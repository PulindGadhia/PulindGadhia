...
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(HERE, "..", "PulindGadhia-ascii.svg")
COLS = 125
ROWS = 62
CELL_W = 8.8
CELL_H = 15.5
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">pulind@github: ~$ ./portrait.sh</text>')
...
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="14">'
             f'pulind@github:~$ whoami <tspan fill="{INK}">Pulind Gadhia</tspan></text>')
